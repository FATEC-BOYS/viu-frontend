// app/l/[token]/viewer/FeedbackPanel.tsx
"use client";

import { useEffect, useRef, useState } from "react";
import { saveAudioFeedback, replyFeedback, updateFeedbackStatus } from "../_actions";
...
export default function FeedbackPanel({
  ...
  async function sendAudio() {
    ...
    const created = await saveAudioFeedback({ token, arteId, blob });
  ...
  return (
    <aside className="space-y-4">
      <div className="border rounded-xl p-3">
        ...
        <p className="text-sm text-muted-foreground">Nosso diferencial: grave e envie seu comentário.</p>

        <div className="mt-3 flex gap-2">
          <button className="px-3 py-2 border rounded" disabled={readOnly || recState==="recording"} onClick={startRec}>
            Gravar
          </button>
          <button className="px-3 py-2 border rounded" disabled={recState!=="recording"} onClick={stopRec}>
            Parar
          </button>
          <button className="px-3 py-2 border rounded" disabled={recState!=="ready"} onClick={sendAudio}>
            Enviar
          </button>
        </div>
              ...
              <div className="mt-2 flex items-center gap-2">
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    if (ok) setItems(prev => prev.map(it => it.id===f.id ? { ...it, status: s } : it));
                  }}
                >
